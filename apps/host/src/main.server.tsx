    ...
    const stream = ReactDOMServer.renderToPipeableStream(
      ...
      </StaticRouter>,
      {
        [callbackName]() {
          ...
          // TODO: fix bundler does not inject style links in the html template
          res.write(
          `${htmlStart.replace(
            '</head>',
            `<link rel="stylesheet" href="src_bootstrap_tsx.css">
            <link rel="stylesheet" href="http://localhost:4300/src_remote-entry_ts.css">
            </head>`
          )}<div id="root">`
        );
        stream.pipe(res);
